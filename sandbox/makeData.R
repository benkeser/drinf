...
makeData <- function(n = n){
    ...
    A0 <- rbinom(n, 1, plogis(2*L0$x.0*L0$x.1))
    L1 <- data.frame(x.2 = L0$x.0*L0$x.1 + 0.2 * A0 + runif(n))
    A1 <- rbinom(n, 1, plogis(2*L0$x.0*L0$x.1 + 0.2*L1$x.2 - 0.1*A0))
    L2 <- rnorm(n, L0$x.0*L0$x.1 + L1$x.2 + A1 + A0)
    return(list(L0 = L0, L1 = L1, L2 = L2, A0 = A0, A1 = A1))
}
...
getTruth <- function(n = 1e6, abar = c(1,1)){
    ...
    L2 <- rnorm(n, L0$x.0*L0$x.1 + L1$x.2 + abar[1] + abar[2])
    return(mean(L2))
}